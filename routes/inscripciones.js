...
router.get("/torneo/listado/:idTorneo", inscripcionListaGetPorTorneo);
...
router.post(
  ...
  [
    ...
    check(
      "pruebasInscripto",
      "Elija al menos una prueba para inscribirse"
    ).isArray({ min: 1 }),
    validarCampos,
  ],
  inscripcionPost
...
router.put(
  ...
  [
    ...
    check("id", "Inscripción no registrada").isMongoId(),
    check("id").custom(existeInscripcionPorId),
    check("pruebasInscripto", "Elija al menos una prueba para inscribirse")
      .optional()
      .isArray({ min: 1 }),
    validarCampos,
  ],
