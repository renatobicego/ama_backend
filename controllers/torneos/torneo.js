...
const torneoPut = async (req, res) => {
  ...
  try {
    ...
    if (resultados)
      torneo.resultados.length > 0 &&
        (await borrarArchivoTorneoFirebase(torneo.resultados));
    if (programaHorario)
      torneo.programaHorario.length > 0 &&
        (await borrarArchivoTorneoFirebase(torneo.programaHorario));
...
const torneoDelete = async (req, res) => {
  ...
  if (req.usuario.role !== "ADMIN_ROLE" && req.usuario.role !== "EDITOR_ROLE") {
    return res.status(403).json({
      msg: "Acceso denegado, solo administradores pueden borrar torneos",
    });
  }
