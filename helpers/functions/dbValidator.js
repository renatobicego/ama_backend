...
const existeInscripcionEnAtleta = async (torneo, { req }) => {
  ...
  if (existeInscripcion) {
    throw new Error(
      `Ya se ha inscripto a este torneo. Si necesita, edite su inscripción en Mi Perfil > Mis Inscripciones`
    );
  }
};

const existePruebaEnUsuario = async (id, atletaId) => {
  const existePruebaRegistradaEnUsuario = await PruebaAtleta.findOne({
    prueba: id,
    atleta: atletaId,
  });
